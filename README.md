...
Values below are **committed snapshots** for grading visibility; **reproduce** by running the commands above on your hardware. Full tables with timestamps live under `results/`.
...
### Agentic report (excerpt)

From `results/agentic_report_20260403_122754.md` (Bios train/val/test caps, same seed):

| Model | E_bio | E1 | E3 | ΔE (E3−E1) |
|-------|-------|-----|-----|------------|
| B_adv | 0.823 | 0.801 | 0.768 | −0.033 |
| Main | 0.701 | 0.574 | 0.574 | 0.000 |

Final-step occupation accuracy (same report): e.g. **B_task ~86.5%**, **Main ~81.9%** — see file for full TABLE 0–5.
...
- **Part 4 — Results:** `results/baseline_comparison.json`, `results/agentic_report_*.md`.
...
Follow the licenses of bundled dependencies (Transformers, Qwen weights, HF datasets). Add a project license file if your course requires one.
